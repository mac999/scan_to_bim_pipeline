...
def get_elevation_from_map(tif_file, elev_map, polygon):
	global args
	poly1 = polygon.buffer(-args.height_building_offset, resolution=16, join_style=2, mitre_limit=1)	# default=-5.  ex) 3
	height1 = get_elevation_from_area(elev_map, poly1)

	poly2 = polygon.buffer(args.height_ground_offest, resolution=16, join_style=2, mitre_limit=1)	# default=10. ex) 3
	height2 = get_elevation_from_area(elev_map, poly2)

	height = np.abs(height1 - height2)
	if height > args.max_building_height:
		return -1.0
...
def save_mesh(tif_file, out_fname, polygons, depth_img, transform_json_file, projection_file, tm, elevation_file):
	...
	try:
		...
		for i in range(len(polygons)):
			...
			pt1 = points_2d[0]
			pt2 = points_2d[N - 1]
			if np.abs(pt2[0]-pt1[0]) < 0.00001 and np.abs(pt2[1]-pt1[1]) < 0.00001:
			...
			def points_2d_to_poly(points, z): # Convert a sequence of 2d coordinates to a polydata with a polygon.
			...
			polygon = points_2d_to_poly(points_2d, z0)
			...
			obj = polygon.extrude((0, 0, z1 - z0), capping=True)
			meshes.append(obj)
...
def create_geometry_from_dtm(infile, outfile):
	...
	calculated_remove_area = args.remove_area / scale_ratio
	print(f'remove area={args.remove_area:.3f}, scale={scale_ratio:.3f}, calculated area={calculated_remove_area:.3f}')
	poly_objs = []
	for i in range(len(contours)):
		if (i > 0) and (len(contours[i])) > 2:
			...
			poly = Polygon(xy)
			area = poly.area
			if area <= calculated_remove_area:
				continue
			poly_objs.append(poly)

	print("Result = ", len(poly_objs))
	polygons = gpd.GeoDataFrame(poly_objs, columns = ['geometry'])
	simplified_polygons = polygons.simplify(args.simplify_factor)    # shapely, https://geopandas.org/en/stable/docs/reference/api/geopandas.GeoSeries.simplify.html

	# save
	save_shp(outfile + ".shp", simplified_polygons, transform_json_file, projection_file, tm, proj, proj_data)     # save_shp_with_elevation("/home/ktw/Projects/pcd_pl/output/volcano_data.txt", "/home/ktw/Projects/pcd_pl/output/volcano_elev.shp")
	save_mesh(infile, outfile + ".ply", simplified_polygons, img, transform_json_file, projection_file, tm, outfile + "_elevation")
